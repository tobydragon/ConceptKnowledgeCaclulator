...
class PointsOffConverterTest {
    ...
    void convertFromPointsOffToTotalPointsTest()throws IOException, CsvException  {
        List<String[]> rowsToConvert = CsvFileLibrary.parseRowsFromFile("src/test/resources/singleUseFiles/pointsOffExample.csv");
        PointsOffConverter.convertFromPointsOffToTotalPoints(rowsToConvert);

        List<String[]> correctRowsExample = CsvFileLibrary.parseRowsFromFile("src/test/resources/singleUseFiles/convertedPointsOffExample.csv");
        assertEquals(correctRowsExample.size(), rowsToConvert.size());
        for (int rowIdx=2; rowIdx < rowsToConvert.size(); rowIdx++){
